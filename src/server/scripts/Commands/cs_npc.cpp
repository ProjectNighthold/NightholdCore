...
public:
    ...
    static bool HandleNpcPlayEmoteCommand(ChatHandler* handler, const char* args)
    {
        ...
        saveEmoteToDb(target->GetDBTableGUIDLow(), emote);

        return true;
    }

	static void saveEmoteToDb(uint64 guidLow, uint32 emotestate)
    {
        PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_CREATURE_ADDON_BY_GUID);

        stmt->setUInt64(0, guidLow); // <-- creature guidlow

        PreparedQueryResult result = WorldDatabase.Query(stmt);

        if (result)
        {
            PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_ADDON_EMOTESTATE); // <-- WORLD_UPD_CREATURE_ADDON_EMOTESTATE needs to be added to the enum and then also added to where ever GetPreparedStatement gets it from.

            stmt->setUInt32(0, emotestate); // <-- Emote state to set 
            stmt->setUInt64(1, guidLow);
        }
        else
        {
            PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_INS_CREATURE_ADDON_EMOTESTATE); // WORLD_INS_CREATURE_ADDON_EMOTESTATE also needs to be added

            stmt->setUInt64(0, guidLow);
            stmt->setUInt32(1, emotestate);
        }
        WorldDatabase.Execute(stmt);
    }
