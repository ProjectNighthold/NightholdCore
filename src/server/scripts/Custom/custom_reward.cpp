...
struct KeyStoneGenerator_npc : public CreatureScript
{
    KeyStoneGenerator_npc() : CreatureScript("KeyStoneGenerator_npc") { }

    bool OnGossipHello(Player* player, Creature* creature) override
    {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "Generate a Mythic Keystone", GOSSIP_SENDER_MAIN, 0);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "Destroy my Mythic Kestone", GOSSIP_SENDER_MAIN, 1);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        case 1:
            if (!player->HasItemCount(138019, 1, true))
                player->DestroyItemCount(138019, 1, true);
